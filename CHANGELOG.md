...
### Added
- **Portable bootstrap source** (`src/platform.kr`, `src/main.c`) — the C runtime preamble now uses `#ifdef _WIN32` gates around all Windows-specific bits (`<windows.h>` vs `<unistd.h>` includes; `Sleep`/`_putenv_s`/`_strdup` vs `usleep`/`setenv`/`strdup` shims). The result is that **a single `src/main.c` builds krakenc on Windows, Linux, and macOS** with `clang -o krakenc src/main.c -Wno-int-conversion <platform-libs>`. `src/main.c` is now committed in the repo as the canonical bootstrap source; CI compiles from it directly with no upstream compiler dependency.
...
- **Generated artifacts now gitignored**: `src/main.ll`, `src/main_runtime.c`, `src/parser.c`, plus `tests/*.c`, `tests/*.ll`, `tests/*_runtime.c`, `tests/*.exe`. `src/main.c` is intentionally tracked as the portable bootstrap source (see "Added" above).
- **CI workflow simplified** (`.github/workflows/ci.yml`, `.github/workflows/release.yml`) — dropped the kraken (Rust) bootstrap dependency. The chain is now `clang src/main.c -o krakenc → tools/run-tests.sh → gen2/gen3 fixed-point check`. The kraken Rust compiler can no longer compile current krakenc (closures with complex captures, large-scale generics, dyn dispatch features it doesn't fully support); the committed `src/main.c` is the canonical bootstrap.
