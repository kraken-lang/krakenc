...
int64_t kr_Point_dist_sq(Point self) {
    return _KR_ADD(self.x * self.x, self.y * self.y);
}
...
kr_str kr_Point_to_string(Point self) {
    return kr_str_concat("(", kr_str_concat(kr_fmt_int((int64_t)(intptr_t)(self.x)), kr_str_concat(",", kr_str_concat(kr_fmt_int((int64_t)(intptr_t)(self.y)), ")"))));
}
...
kr_str kr_fizzbuzz(int64_t n) {
    ...
    return kr_fmt_int((int64_t)(intptr_t)(n));
}
...
void kr_assert_eq_int(kr_str label, int64_t expected, int64_t actual) {
    if (_KR_EQ(expected, actual)) {
        ...
        kr_puts(kr_str_concat("  FAIL: ", kr_str_concat(label, kr_str_concat(" expected=", kr_str_concat(kr_fmt_int((int64_t)(intptr_t)(expected)), kr_str_concat(" actual=", kr_fmt_int((int64_t)(intptr_t)(actual))))))));
    }
}
...
int64_t kr_main() {
    ...
    kr_puts("[For loop]");
    __auto_type sum = 0;
    for (int64_t i = 0; i < 10; i = _KR_ADD(i, 1)) {
        sum = _KR_ADD(sum, i);
    }
