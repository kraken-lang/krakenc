...
void kr_assert_eq(kr_str label, int64_t expected, int64_t actual) {
    if (_KR_EQ(expected, actual)) {
        ...
        kr_puts(kr_str_concat("  FAIL: ", kr_str_concat(label, kr_str_concat(" exp=", kr_str_concat(kr_fmt_int((int64_t)(intptr_t)(expected)), kr_str_concat(" got=", kr_fmt_int((int64_t)(intptr_t)(actual))))))));
    }
}
...
int64_t kr_complex_expr(int64_t a, int64_t b, int64_t c) {
    return (_KR_ADD(a, b)) * c - _KR_ADD((a * b) / (_KR_ADD(c, 1)), (a % (_KR_ADD(b, 1))));
}

kr_str kr_chained_calls(kr_str s) {
    return kr_str_concat(kr_str_concat(kr_str_concat("[", s), "]"), kr_str_concat(" len=", kr_fmt_int((int64_t)(intptr_t)(kr_strlen(s)))));
}
...
int64_t kr_sum_big(BigStruct s) {
    return _KR_ADD(_KR_ADD(_KR_ADD(_KR_ADD(_KR_ADD(s.a, s.b), s.c), s.d), s.e), s.f);
}
...
kr_str kr_join_strings(kr_str sep, int64_t count) {
    ...
    for (int64_t i = 0; i < count; i++) {
        kr_vec_string_push(v, kr_fmt_int((int64_t)(intptr_t)(i)));
    }
    __auto_type result = "";
    __auto_type i = 0;
