...
Vec2 kr_Vec2_add(Vec2 self, Vec2 other) {
    return (Vec2){.x = _KR_ADD(self.x, other.x), .y = _KR_ADD(self.y, other.y)};
}

int64_t kr_main() {
    __auto_type a = kr_new_vec2(3, 4);
    __auto_type b = kr_new_vec2(1, 2);
    kr_puts(kr_str_concat("mag_sq(3,4)=", kr_fmt_int((int64_t)(intptr_t)(kr_Vec2_mag_sq(a)))));
    __auto_type c = kr_Vec2_add(a, b);
    kr_puts(kr_str_concat("add=(", kr_str_concat(kr_fmt_int((int64_t)(intptr_t)(c.x)), kr_str_concat(",", kr_str_concat(kr_fmt_int((int64_t)(intptr_t)(c.y)), ")")))));
    return 0;
}
